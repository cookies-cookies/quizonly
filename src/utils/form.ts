...
export function isEduEmailValid(email: string) {
  // 只允许 .edu.cn 结尾的教育邮箱或 @gmail.com 的谷歌邮箱
  const eduPattern = /^[^\s@]+@[^\s@]+\.edu\.cn$/;
  const gmailPattern = /^[^\s@]+@gmail\.com$/;
  return (eduPattern.test(email) || gmailPattern.test(email)) && email.length <= 255;
}
